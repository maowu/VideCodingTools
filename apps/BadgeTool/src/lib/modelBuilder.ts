...
import { resolveShapeSettings } from "@/lib/shapeSettings";
...
export function buildMedalGroup(
  ...
  quality: BuildQuality = "export",
): THREE.Group {
  const isPreview = quality === "preview";
  const effectiveSettings: MedalSettings = isPreview && settings.dome
    ? {
        ...settings,
        dome: {
          ...settings.dome,
          segments: Math.min(settings.dome.segments ?? 22, PREVIEW_MAX_DOME_SEGMENTS),
        },
      }
    : settings;
  ...
  for (const record of records) {
    ...
    const curveSegments = isPreview
      ? Math.min(shapeSettings.curveSegments, PREVIEW_MAX_CURVE_SEGMENTS)
      : shapeSettings.curveSegments;
    const bevelSegments = isPreview
      ? Math.min(shapeSettings.bevelSegments, PREVIEW_MAX_BEVEL_SEGMENTS)
