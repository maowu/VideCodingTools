...
import { getShapeLabel, getSvgColor, resolveShapeSettings } from "@/lib/shapeSettings";
import { summarizeSvgPaths } from "@/lib/svgSummary";
import {
  createWorkDocument,
  createWorkId,
...
import { getWorkDocument, saveWorkDocument } from "@/lib/workStorage";
import { generateWorkSnapshot, needsSnapshot } from "@/lib/snapshot";
import { requestIdleTask } from "@/lib/idle";
import { pngFileToSvg } from "@/lib/pngToSvg";
import type {
  CanvasSettings,
  DomeSettings,
  MaterialId,
...
function MaterialPreviewSwatch({ src }: { src: string }) {
  return (
    <span
      aria-hidden="true"
    ...
    <div className="control-row">
      ...
      <div className="material-select" ref={rootRef}>
        <button
          ...
          type="button"
        >
          <MaterialPreviewSwatch src={selectedMaterial.previewSrc} />
            ...
            {MATERIAL_PRESETS.map((material) => {
              ...
              return (
                <button
                  ...
                  type="button"
                >
                  <MaterialPreviewSwatch src={material.previewSrc} />
...
function getRectCornerRadius(summary: SvgPathSummary | undefined) {
  return getSummaryNumber(summary, "rx", getSummaryNumber(summary, "ry", 0));
}

const WINDING_OPTIONS: SegmentOption<ShapeWindingMode>[] = [
  { label: "SVG", value: "svg" },
  { label: "CW", value: "solidCw" },
  { label: "CCW", value: "solidCcw" },
];
...
export function MedalWorkbench() {
  const navigate = useNavigate();
  const { id: initialWorkId } = useParams<{ id: string }>();
  ...
  const [tracedSourceName, setTracedSourceName] = useState<string | null>(null);
  ...
  const [editingLayerPathIndex, setEditingLayerPathIndex] = useState<number | null>(null);
  ...
  const saveCurrentWork = useCallback(
    async (quiet = false, titleOverride?: string) => {
      let doc = createCurrentWorkDocument(
        createTimestamp(),
        workMeta.snapshot,
        titleOverride,
      );
      ...
      if (needsSnapshot(doc)) {
        try {
          ...
          doc = { ...doc, preview: { snapshot } };
        } catch {
          doc = { ...doc, preview: { snapshot: workMeta.snapshot } };
        }
      }

      await saveWorkDocument(doc);
      ...
      setWorkMeta({
        ...
        snapshot: doc.preview.snapshot,
      });
      setHasPersistedWork(true);

      if (!quiet) {
        setStatus(`Saved ${doc.document.title}`);
      }
  ...
  function applyWorkDocument(doc: WorkDocument) {
    const asset = getPrimarySvgAsset(doc);

    setSvgText(asset.text);
    setFileName(asset.fileName);
    setSettings(doc.scene.settings);
    setSelectedPathIndexes(doc.editorState.selectedPathIndexes);
    setHoveredPathIndex(null);
    setEditingLayerPathIndex(null);
    setLayerNameDraft("");
    selectionAnchorRef.current = doc.editorState.selectedPathIndexes[0] ?? null;
    setWorkMeta({
      id: doc.document.id,
      title: doc.document.title,
      createdAt: doc.document.createdAt,
    ...
    setLastSavedSignature(
      createWorkPayloadSignature({
        ...
        svgText: asset.text,
        settings: doc.scene.settings,
        selectedPathIndexes: doc.editorState.selectedPathIndexes,
      }),
    );
  }
  ...
  function commitTitleEdit() {
    ...
    if (nextTitle !== workMeta.title) {
      setWorkMeta((current) => ({ ...current, title: nextTitle }));
  ...
  async function confirmBeforeReplacingWork(
    ...
    const shouldSave = window.confirm(
      `Current work has unsaved changes. ${actionLabel} will discard current edits. Save first?`,
    );
    ...
    return window.confirm(`Continue ${actionLabel} without saving? Current edits will be lost.`);
  }
  ...
  async function goHome() {
    ...
    const canLeave = await confirmBeforeReplacingWork("going home", pendingTitle);
  ...
  function updateShapes(
    ...
    setSettings((current) => {
      ...
      return { ...current, shapeSettings: nextShapeSettings };
    });
  }
  ...
  function handleLayerNameKeyDown(event: ReactKeyboardEvent<HTMLInputElement>) {
    if (event.key === "Enter") {
      event.preventDefault();
      event.currentTarget.blur();
      return;
  ...
  function resetActiveSvgAdjustment<K extends keyof ShapeSvgAdjustments>(key: K) {
    ...
    const nextAdjustments = { ...activeShapeSettings.svgAdjustments };
    delete nextAdjustments[key];

    updateShapes([activeSummary.pathIndex], { svgAdjustments: nextAdjustments });
  }
    ...
    getWorkDocument(initialWorkId)
      .then((doc) => {
        ...
        if (!doc) {
          ...
          setWorkMeta({ id: initialWorkId, title, createdAt, snapshot: null });
          setTitleDraft(title);
        ...
        applyWorkDocument(doc);
        setStatus(`Loaded ${doc.document.title}`);
        setIsDocumentReady(true);
      })
      .catch(() => {
        if (active) {
  ...
  useEffect(() => {
    // Only auto-save works that have been explicitly saved at least once.
    // Prevents default "2026-03" from being silently persisted for every new work the user opens.
    if (!isDocumentReady || !hasPersistedWork || !hasUnsavedChanges()) {
      return;
    }
  ...
  }, [hasPersistedWork, hasUnsavedChanges, isDocumentReady, runAutoSave]);
  ...
  function resetForNewWork(nextFileName: string, preserveCurrentDraft = false) {
    ...
    selectionAnchorRef.current = null;
    setLastSavedSignature("");
    setTracedSourceName(null);
  }

  function discardTracedResult() {
    resetForNewWork(DEFAULT_FILE_NAME, false);
    setSvgText(DEFAULT_SVG);
    setFileName(DEFAULT_FILE_NAME);
    setSettings(DEFAULT_SETTINGS);
    setStatus("Discarded traced result");
  }
  ...
  async function loadSvgFile(file: File) {
    ...
    if (!isPng && !isSvg) {
      setStatus("Only SVG or PNG files are accepted");
      return;
    }

    const actionLabel = isPng ? "tracing PNG to SVG" : "importing new SVG";
    const canReplace = await confirmBeforeReplacingWork(actionLabel);
    if (!canReplace) {
      setStatus("Import cancelled");
      return;
    }

    let svgFileName = file.name;
    let text: string;
    ...
    if (isPng) {
      ...
      try {
        text = await pngFileToSvg(file);
        svgFileName = file.name.replace(/\.png$/i, ".svg");
      } catch {
        setStatus("PNG tracing failed");
    ...
    resetForNewWork(svgFileName, !hasPersistedWork);
    setSvgText(text);
    setFileName(svgFileName);
    setSettings((current) => ({ ...current, shapeSettings: {} }));
    setTracedSourceName(isPng ? file.name : null);
    setStatus(`Loaded ${svgFileName}`);
  }
          ...
          <div className="panel-scroll">
            <section className="panel-section">
              ...
              <div className="control-stack">
                <button
                  className={
                    isDragActive ? "upload-dropzone drag-active" : "upload-dropzone"
                  }
                ...
                >
                  ...
                  <span>{isConverting ? "Tracing PNG…" : "Drop SVG / PNG"}</span>
                  {!isConverting && <small>or click to upload</small>}
                </button>
                <input
                ...
                {fileName !== DEFAULT_FILE_NAME ? (
                  <button
                    className="text-button danger discard-trace-button"
                    onClick={discardTracedResult}
                    title={
                      tracedSourceName
                        ? `Discard traced result from ${tracedSourceName}`
                        : `Remove ${fileName}`
                    }
                    type="button"
                  >
                    <Trash2 size={13} />
                    {tracedSourceName ? "Discard trace" : "Remove file"}
                  </button>
                ) : null}
              </div>
            </section>
        ...
        <section className="viewport-panel">
          <div className="canvas-settings">
            ...
            <button
              ...
              className={
                settings.canvas.showGrid ? "compact-switch active" : "compact-switch"
              }
              onClick={() =>
